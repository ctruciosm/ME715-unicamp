...
- Lembre-se que $\rho_s = \dfrac{\mathbb{C}ov(u_t, u_{t+2})}{\sqrt{\mathbb{V}(u_t) \times \mathbb{V}(u_{t+s})}}$. - Assim, sob homocedasticidade, $\rho_s = \dfrac{\mathbb{E}(u_t, u_{t+s})}{\sigma_u^2}$
