...
- Seja $z_1$ a média dos salários da indústria
...
Note que ambas as equações descrevem relações diferentes. A primeira é uma equação comportamental dos trabalhadores e a segunda é uma equação comportamental dos fazendeiros.
...
Assim que especificarmos os outros fatores, teremos um modelo de equações simultâneas!
...
<center>
[Seja que estamos interessados em apenas uma parte ou no sistema como um todo, a interação de todas as variáveis no modelo terão um papel importante na interpretação e estimação do modelo.]{style="color:red;"}
</center>
...
<center>
[Se $y_2$ e $u_1$ são não correlacionados, podemos aplicar MQO sem problemas. Vejamos se $\mathbb{C}or(y_2, u_1) = 0$.]{style="color:blue;"}
</center>
...
(note que, se fosse nosso objetivo, poderimos utilizar MQO para estimar $\pi_{21}$ e $\pi_{22}$).
...
- Contudo, $$\log(wage) = \beta_{20} + \alpha_2 hours + \beta_{21} educ + \beta_{22}exper + \beta_{23} exper^2 + u_2$$
 ($\log(wage)$ é endógena!)
- Se a equação da oferta da mão de obra for identificável, podemos estimá-la por MQ2E, onde os instrimentos consistirão nas variáveis exógenas que aparecem em cada equação.
...
$$\underbrace{K - k}_{2} \geq \underbrace{m - 1}_{0}?$$
...
equacao02 <- ivreg(log(wage) ~ hours + educ + exper + I(exper^2) | educ + age + kidslt6 + nwifeinc + exper + I(exper^2), data = mroz)
```
...
1. Ajuste MQ2E  no sistema de equações.
2. Utilize os resíduos para estimar a matriz de covariância.
3. Aplique MQGF.
