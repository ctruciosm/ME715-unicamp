...
- Na aula de hoje focaremos em outro estimador: **o método dos momentos generalizado** (GMM)
    *   Tem propriedades asintóticas desejáveis
    *   Muitos dos estimadores visto até agora são casos particulares do GMM.
...
- GMM precisa alguma habilidade em programação para implementá-lo
...
<center>
[Uma vez definidos os momentos populacionais e amostrais, podemos entender a ideia por trás do MM. **Se queremos estimar o momento populacional (ou uma função de momentos populacioonais), simplesmente utilizamos o correspondente momento amostral (ou funções dos momentos amostrais**]{style="color:red;"}
</center>
...
$$\mathbb{V}(X) = \mu_2 - \mu_1^2 \quad \rightarrow \quad \hat{\mathbb{V}}(X) = \dfrac{1}{n}\sum x^2 - \Big [\dfrac{1}{n} \sum x \big]^2 = \dfrac{1}{n} \sum (x - \bar{x})^2$$
...
Isto implica que $$\mathbb{E}(X'\epsilon) = \mathbb{E}(X'[Y - X\beta]) = 0.$$
...
Pelo MM, $$\dfrac{1}{n} X'[Y - X \beta] = 0 \quad \rightarrow \hat{\beta}_{MM} = (X'X)^{-1}X'Y = \hat{\beta}_{MQO}$$
...
- MQO é inconsistente
- A ideia é encontrar instrumentos $\textbf{Z}$ que sejam correlacionados com X mas $\mathbb{E} (\textbf{Z}'\epsilon) = 0$
...
- Nesta formulação, restrições que são estimadas com menos menor precisão recebem menor peso do que aquelas estimadas com maior precisão.
...
Um bom estimador para $\hat{\textbf{V}} = \dfrac{\sigma^2}{n} \textbf{Z}'\textbf{Z}$, o que nos leva ao seguinte estimador:
...
2. $m(\theta) = \dfrac{1}{n} [\textbf{Z}'(y - \textbf{X}\beta)]$, então temos que minimizar $$\Big[\dfrac{1}{n} [\textbf{Z}'(y - \textbf{X}\beta) ] \Big]' \textbf{W}^{-1} \Big[\dfrac{1}{n} [\textbf{Z}'(y - \textbf{X}\hat{\beta}) ] \Big],$$ em que $\textbf{W}$ é um estimador consistente para $\mathbb{V}( \dfrac{1}{n} [\textbf{Z}'(y - \textbf{X}\beta)]) = \dfrac{\hat{\sigma}^2}{n} \textbf{Z}' \textbf{Z}.$
